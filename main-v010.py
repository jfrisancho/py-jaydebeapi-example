...
Examples:
  ...
  Unattended mode:
    ...
    python main.py --approach SCENARIO --method PREDEFINED --coverage-target 0.15 --fab M16 --verbose
        """
    )
...
def unattended_mode(args) -> None:
    ...
    try:
        ...
        if not args.fab:
            raise ValueError("--fab is required for unattended mode")
        ...
        tag = generate_tag(approach, method, args.coverage_target, args.fab, toolset)
        ...
        if args.verbose:
            ...
            print(f"Starting {approach.value} {method.value} analysis for {args.fab} (target: {args.coverage_target:.1%})")
