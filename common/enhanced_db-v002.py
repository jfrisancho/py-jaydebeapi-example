...
class Database:
    ...
    def update(self, sql: str, params: Optional[List[Any]] = None) -> int:
        ...
        try:
            ...
            except jaydebeapi.DatabaseError as rollback_error:
                if not self.unattended:
    ...
    def insert_returning(self, sql: str, params: Optional[List[Any]] = None) -> Any:
        """
        Execute an INSERT statement with RETURNING clause and return the returned value.
        Typically used with INSERT INTO ... RETURNING id; to get the inserted row's ID.
        ...
        Returns:
            The value returned by the RETURNING clause (typically the new ID)
        """
        if self.verbose:
            print(f"Executing insert with returning: {sql}")
            if params:
                print(f"Insert parameters: {params}")
        ...
        try:
            with self.cursor() as cur:
                ...
                # Fetch the returned value from RETURNING clause
                result = cur.fetchone()
                self._conn.commit()
                
                if result:
                    returned_value = result[0]  # Usually the ID is in the first column
                    if self.verbose:
                        print(f"Insert successful, returned value: {returned_value}")
                    return returned_value
                else:
                    if self.verbose:
                        print("Insert executed but no value returned")
                    return None
                
        except jaydebeapi.DatabaseError as e:
            if not self.unattended:
                print(f"ERROR: Insert with returning failed: {e}")
            # Attempt rollback
            try:
                self._conn.rollback()
                if self.verbose:
