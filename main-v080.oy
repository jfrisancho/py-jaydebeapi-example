...
from enums import Approach, Method, RunStatus, Building, ExecutionMode
...
Examples:
  Default (quick random test with default settings):
    ...
    python main.py --building M16                  # specify building
  ...
  Random approach (specific tests):
    python main.py -a RANDOM --building M16 --toolset "TOOLSET_001"
  ...
  Silent unattended mode (for scripts/automation):
    python main.py --building M16 --unattended
    ...
    parser.add_argument(
        ...
        help='Building identifier for RANDOM approach (e.g., M16, M15) - ignored for SCENARIO approach'
    )
    
    parser.add_argument(
        '--toolset',
        type=str,
        help='Toolset identifier for RANDOM approach (e.g., TOOLSET_001) - ignored for SCENARIO approach'
    )
    
    parser.add_argument(
        '--phase',
        type=str,
        choices=['PHASE1', 'PHASE2', 'A', 'B'],
        help='Phase identifier for RANDOM approach (PHASE1/A, PHASE2/B) - ignored for SCENARIO approach'
    )
...
def fetch_available_buildings(db: Database) -> List[str]:
    """Get available buildings from database."""
    try:
        sql = "SELECT code FROM tb_buildings WHERE is_active = TRUE ORDER BY code"
        results = db.query(sql)
        buildings = [row[0] for row in results] if results else []
        
        # Add default buildings if none found
        if not buildings:
            buildings = [b.value for b in Building]
        
        return buildings
    except Exception as e:
        return [b.value for b in Building]  # Default options
...
def fetch_available_toolsets(db: Database, building_code: str) -> List[str]:
    """Get available toolsets for a specific building."""
    try:
        sql = "SELECT DISTINCT code FROM tb_toolsets WHERE building_code = ? AND is_active = TRUE ORDER BY code"
        results = db.query(sql, [building_code])
...
def fetch_available_phases(db: Database, building_code: str) -> List[str]:
    """Get available phases for a specific building."""
    try:
        sql = "SELECT DISTINCT phase FROM tb_toolsets WHERE building_code = ? AND is_active = TRUE ORDER BY phase"
        results = db.query(sql, [building_code])
...
def validate_approach_specific_args(approach: Approach, args) -> None:
    ...
    if approach == Approach.SCENARIO:
        # SCENARIO approach doesn't use building/toolset/coverage-target/method/phase
        if args.building:
            print(f"Warning: --building is ignored for SCENARIO approach")
        if args.toolset:
...
def normalize_phase(phase_str: str) -> str:
    ...
    try:
        normalized = Phase.normalize(phase_str)
        return normalized.value
    except ValueError:
        return phase_str  # Return as-is if normalization fails
...
def print_configuration_summary(config: RunConfig):
    ...
    if config.approach == Approach.RANDOM:
        ...
        print(f"Building: {config.building_code}")
        if config.phase:
...
def print_run_summary(result: RunResult, verbose: bool = False):
    ...
    print(f"Building: {result.building_code}")
...
def unattended_mode(args) -> None:
    ...
    try:
        ...
        if approach == Approach.RANDOM:
            building_code = args.building or ""
            toolset = args.toolset or ""
            phase = normalize_phase(args.phase or "")
            
            # Get default building if not provided (silently)
            if not building_code:
                db = Database()
                try:
                    available_buildings = fetch_available_buildings(db)
                    if available_buildings:
                        building_code = available_buildings[0]
                    else:
...
def default_mode(args) -> None:
    ...
    try:
        ...
        if approach == Approach.RANDOM:
            ...
            # Get default building if not provided
            if not building_code:
                db = Database()
                try:
                    available_buildings = fetch_available_buildings(db)
                    if available_buildings:
                        building_code = available_buildings[0]
                        if args.verbose:
                            print(f"No building specified, using default: {building_code}")
                    else:
                        building_code = "M16"
                        if args.verbose:
                            print(f"No buildings found in database, using: {building_code}")
                except Exception as e:
                    building_code = "M16"
                    if args.verbose:
                        print(f"Could not retrieve buildings from database, using: {building_code}")
                finally:
                    db.close()
        ...
        if args.verbose:
            ...
            if approach == Approach.RANDOM:
                building_display = f" for {building_code}" if building_code else ""
                print(f"Starting {approach.value} {method.value} analysis{building_display} (target: {coverage_target:.1%})")
            else:
