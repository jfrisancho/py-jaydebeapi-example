...
def load_verification(db: Database) -> bool:
    ...
    """
    print("\nPerforming comprehensive load verification...")
    ...
    if result:
        total_pocs, unique_equipments, used_pocs, unused_pocs, unique_utilities = result[0]
    ...
    if duplicate_nodes:
        ...
        for node_id, count in duplicate_nodes[:5]:  # Show first 5
            print(f"     - Node {node_id}: {count} duplicates")
        if len(duplicate_nodes) > 5:
            print(f"     - ... and {len(duplicate_nodes) - 5} more")
    else:
    ...
    if duplicate_codes:
        ...
        verification_passed = False
        for equipment_id, code, count in duplicate_codes[:5]:  # Show first 5
            print(f"     - Equipment {equipment_id}, Code {code}: {count} duplicates")
        if len(duplicate_codes) > 5:
            print(f"     - ... and {len(duplicate_codes) - 5} more")
    else:
        print("   ✓ No duplicate equipment_id+code combinations found")
    
    # 3. CRITICAL: Vertices vs POCs count verification
    print("\n3. Critical Verification - Vertices vs POCs Count:")
    vertices_poc_sql = """
        SELECT 
            e.id as equipment_id,
            e.guid as equipment_guid,
            e.name as equipment_name,
            e.vertices as expected_vertices,
            COUNT(p.id) as actual_pocs,
            e.vertices - COUNT(p.id) as difference
        FROM tb_equipments e
        LEFT JOIN tb_equipment_pocs p ON e.id = p.equipment_id
        WHERE e.is_active = TRUE
        GROUP BY e.id, e.guid, e.name, e.vertices
        HAVING e.vertices != COUNT(p.id)
        ORDER BY ABS(e.vertices - COUNT(p.id)) DESC, e.guid
    """
    
    mismatched_counts = db.query(vertices_poc_sql)
    
    if mismatched_counts:
        print(f"   ✗ CRITICAL: Found {len(mismatched_counts)} equipments with vertices/POCs count mismatch")
        verification_passed = False
        
        # Show detailed breakdown
        over_pocs = [row for row in mismatched_counts if row[5] < 0]  # difference < 0 means more POCs than vertices
        under_pocs = [row for row in mismatched_counts if row[5] > 0]  # difference > 0 means fewer POCs than vertices
        
        if over_pocs:
            print(f"\n   Equipments with MORE POCs than vertices ({len(over_pocs)}):")
            for equipment_id, guid, name, vertices, pocs, diff in over_pocs[:10]:
                print(f"     - {guid[:12]}... ({name[:20]}...): Expected {vertices}, Got {pocs} ({abs(diff)} extra)")
            if len(over_pocs) > 10:
                print(f"     - ... and {len(over_pocs) - 10} more")
        
        if under_pocs:
            print(f"\n   Equipments with FEWER POCs than vertices ({len(under_pocs)}):")
            for equipment_id, guid, name, vertices, pocs, diff in under_pocs[:10]:
                print(f"     - {guid[:12]}... ({name[:20]}...): Expected {vertices}, Got {pocs} ({diff} missing)")
            if len(under_pocs) > 10:
                print(f"     - ... and {len(under_pocs) - 10} more")
        
        # Summary statistics
        total_expected = sum(row[3] for row in mismatched_counts)
        total_actual = sum(row[4] for row in mismatched_counts)
        print(f"\n   Summary for mismatched equipments:")
        print(f"     Total expected vertices: {total_expected}")
        print(f"     Total actual POCs: {total_actual}")
        print(f"     Net difference: {total_actual - total_expected}")
        
    else:
        print("   ✓ EXCELLENT: All equipments have POC count matching their vertices count")
    
    # 4. Show equipments with correct vertices/POCs match
    correct_counts_sql = """
        SELECT 
            COUNT(*) as correct_equipments,
            SUM(e.vertices) as total_vertices_matched,
            AVG(e.vertices) as avg_vertices_per_equipment
        FROM tb_equipments e
        LEFT JOIN tb_equipment_pocs p ON e.id = p.equipment_id
        WHERE e.is_active = TRUE
        GROUP BY e.id, e.vertices
        HAVING e.vertices = COUNT(p.id)
    """
    
    correct_result = db.query(correct_counts_sql)
    if correct_result:
        correct_count = len(db.query("""
            SELECT e.id
            FROM tb_equipments e
            LEFT JOIN tb_equipment_pocs p ON e.id = p.equipment_id
            WHERE e.is_active = TRUE
        ...
        if correct_count > 0:
            total_vertices = db.query("SELECT SUM(vertices) FROM tb_equipments WHERE is_active = TRUE")[0][0]
            print(f"\n   ✓ {correct_count} equipments have correct vertices/POCs match")
            print(f"   ✓ These represent {total_vertices - sum(row[3] for row in mismatched_counts) if mismatched_counts else total_vertices} correctly matched vertices")
    
    # 5. POC usage analysis
    print("\n4. POC Usage Analysis:")
    usage_sql = """
        SELECT 
            COUNT(CASE WHEN is_used = 1 THEN 1 END) as used_pocs,
            COUNT(CASE WHEN is_used = 0 THEN 1 END) as unused_pocs,
            ROUND(100.0 * COUNT(CASE WHEN is_used = 1 THEN 1 END) / COUNT(*), 2) as usage_percentage
        FROM tb_equipment_pocs
    """
    
    usage_result = db.query(usage_sql)
    if usage_result:
        used, unused, percentage = usage_result[0]
        print(f"   Used POCs: {used} ({percentage}%)")
        print(f"   Unused POCs: {unused} ({100 - percentage:.2f}%)")
    
    # 6. Top utilities analysis
    print("\n5. Utility Distribution (Top 10):")
    utilities_sql = """
        SELECT 
            utility,
            COUNT(*) as poc_count,
        ...
        FROM tb_equipment_pocs
        WHERE is_used = 1 AND utility IS NOT NULL
        GROUP BY utility
        ORDER BY COUNT(*) DESC
        LIMIT 10
    """
    
    utilities_result = db.query(utilities_sql)
    if utilities_result:
        for utility, count, percentage in utilities_result:
            print(f"   {utility}: {count} POCs ({percentage}%)")
    else:
    ...
    if verification_passed:
        ...
        print("\nRecommended actions:")
        print("1. Check source data for missing or duplicate POCs")
        print("2. Verify equipment vertices count in source data")
        print("3. Review POC generation logic for multi-dimensional cases")
