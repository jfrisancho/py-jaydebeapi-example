...
def generate_poc_code(eq_poc_no: str, utility: str, nwo_type: str) -> str:
    ...
    Returns:
        Formatted POC code (POC01, POC02, IN01, OUT01, etc.)
    """
    if not eq_poc_no:
        return "POC01"  # Default
    
    # Clean the input
    eq_poc_clean = str(eq_poc_no).strip().upper()
    
    # Try to extract number from eq_poc_no
    import re
    number_match = re.search(r'(\d+)', eq_poc_clean)
    number = number_match.group(1).zfill(2) if number_match else "01"
    
    # Determine prefix based on type and utility
    if nwo_type and 'IN' in nwo_type.upper():
        return f"IN{number}"
    elif nwo_type and 'OUT' in nwo_type.upper():
        return f"OUT{number}"
    elif eq_poc_clean.startswith('IN'):
        return f"IN{number}"
    elif eq_poc_clean.startswith('OUT'):
        return f"OUT{number}"
    else:
        return f"POC{number}"
