...
class NetworkPathFinderDFS:
    ...
    def load_network_data(self, utility_no=0, toolset_id=0, eq_poc_no=''):
        ...
        sql = (
            "SELECT node_id, data_code, utility_no, toolset_id, COALESCE(eq_poc_no,''), net_obj_type "
            "FROM nw_nodes"
        )
        if where:
    ...
    def enumerate_downstream_paths(
        ...
        def dfs(curr_node_id: int, path_links: List, path_nodes: List[int], total_cost: float, visited: Set[int]):
            ...
            # Find next neighbors passing filters
            next_neighbors = []
            ...
            # (1) End if leaf (no outgoing links at all)
            ...
            # (3) End if there ARE outgoing links, but none pass the filters (filtered dead end)
