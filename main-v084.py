...
def fetch_selected_choice(
    ...
    Args:
        ...
        default: Default choice index (1-based, 0 means no default)
        required: Whether a choice is required
        allow_none: Whether to add a 'NONE' option
        allow_all: Whether to add an 'ALL' option
        allow_skip: Whether to add a 'SKIP' option that returns None
        
    Returns:
        Selected choice string, 'NONE', 'ALL', or None
    """
    # Check if choices are empty
    if not choices:
        if allow_skip or not required:
            return None
        else:
    ...
    # Build the display choices list
    display_choices = choices.copy()
    if allow_none:
        display_choices.append('NONE')
    if allow_all:
        display_choices.append('ALL')
    if allow_skip:
        display_choices.append('SKIP')
    
    # Display all options
    for i, choice in enumerate(display_choices, 1):
        marker = " (default)" if i == default else ''
        print(f'  {i}. {choice}{marker}')
    
    while True:
        try:
            s = f'\nEnter choice (1-{len(display_choices)})'
            if default:
                s += f' [default: {default}]'
            s += ': '
            ...
            if not value:
                if default:
                    selected = display_choices[default-1]
                    # Return None if SKIP was selected as default
            ...
            if 0 <= choice_idx < len(display_choices):
                selected = display_choices[choice_idx]
                # Return None if SKIP was selected
                return None if selected == 'SKIP' else selected
            else:
                print(f"Invalid choice. Please enter a number between 1 and {len(display_choices)}.")
        except ValueError:
...
def collect_random_config_params(args, interactive: bool = False) -> Tuple[str, str, str]:
    ...
    if interactive:
        ...
        try:
            # Get fab
            available_fabs = fetch_available_fabs(db)
            building_code = fetch_selected_choice(
...
def interactive_mode():
    ...
    try:
        ...
        # 6. Verbose option
        verbose_choice = fetch_selected_choice(
