...
class NetworkPathFinder:
    ...
    def find_leaf_nodes(self, ignore_node_id: int = 0) -> Set[int]:
        """
    ...
    def find_downstream_paths(self, start_node_id: int, ignore_node_id: int = 0) -> List[PathResult]:
        """
        Find all downstream paths from start_node_id to leaf nodes using Dijkstra.
        ...
        Returns:
            List of PathResult objects for each reachable leaf node
        """
        if not self.loaded:
            raise RuntimeError('Network data not loaded. Call load_network_data() first.')
        ...
        # Find all leaf nodes
        leaf_nodes = self.find_leaf_nodes(ignore_node_id)
        if not leaf_nodes:
            print('No leaf nodes found in network')
            return []
        
        print(f'Found {len(leaf_nodes)} leaf nodes to search for')
        ...
        # Build paths to reachable leaf nodes
        paths = []
        path_id = 1
        
        for leaf_node in leaf_nodes:
            if leaf_node not in distances:
                continue  # Unreachable leaf
            ...
            if path_links:  # Only add if path exists
                paths.append(PathResult(
                    path_id=path_id,
                    start_node_id=start_node_id,
                    end_node_id=leaf_node,
                    total_cost=distances[leaf_node],
                    links=path_links
                ))
                path_id += 1
    ...
    def analyze_node_flags(self, paths: List[PathResult]) -> Dict[Tuple[int, int], str]:
        """
        ...
        for path in paths:
            ...
            for i, node_id in enumerate(path_nodes):
                ...
                if i == 0:
                    ...
                    # Last node in path
                    # Check if it's a true leaf (no outgoing connections in entire network)
                    if node_outgoing_count.get(node_id, 0) == 0:
                        node_flags[flag_key] = 'L'  # Leaf node
                    else:
                        node_flags[flag_key] = 'E'  # End of this path but not a leaf
                else:
...
def find_network_downstream(db, start_node_id: int, ignore_node_id: int = 0,
    ...
    try:
        ...
        paths = pathfinder.find_downstream_paths(start_node_id, ignore_node_id)
