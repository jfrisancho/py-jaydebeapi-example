...
-- Complete Equipment Schema with PoCs

CREATE TABLE tb_toolsets (
    code VARCHAR(64) PRIMARY KEY,
    fab VARCHAR(10) NOT NULL,
    phase VARCHAR(8) NOT NULL,          -- PHASE1, PHASE2 (or A/B mapped to 1/2)
    name VARCHAR(128) NOT NULL,
...
-- Equipment: Enhanced equipment definitions
CREATE TABLE tb_equipment (
    id INTEGER AUTO_INCREMENT PRIMARY KEY,
    ...
    guid VARCHAR(64) NOT NULL,          -- Unique equipment identifier
    node_id INTEGER NOT NULL,           -- Virtual node in nw_nodes representing the equipment
    kind VARCHAR(32),                   -- PRODUCTION, PROCESSING, SUPPLY, EXHAUST, etc.
    ...
    INDEX idx_equipment_node (node_id),
    UNIQUE KEY uk_equipment_guid (guid)
);

-- Equipment Points of Contact (PoCs)
CREATE TABLE tb_equipment_pocs (
    id INTEGER AUTO_INCREMENT PRIMARY KEY,
    equipment_id INTEGER NOT NULL,
    code VARCHAR(16) NOT NULL,          -- PoC identifier (e.g., POC01, POC02, IN01, OUT01)
    node_id INTEGER NOT NULL,           -- Actual network node ID from nw_nodes
    
    -- PoC characteristics
    utility_code VARCHAR(32),           -- Utility type (e.g., N2, CDA, PW, etc.) - NULL if unused
    flow_direction VARCHAR(8),          -- IN, OUT, BIDIRECTIONAL - NULL if unused
    is_used BOOLEAN DEFAULT FALSE,      -- Whether this PoC is currently in use
    
    -- PoC metadata
    description VARCHAR(256),
    priority INTEGER DEFAULT 0,        -- Priority for selection (higher = more preferred)
    is_active BOOLEAN DEFAULT TRUE,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    ...
    INDEX idx_poc_utility (utility_code, flow_direction),
    INDEX idx_poc_used (is_used, is_active),
    UNIQUE KEY uk_equipment_poc_code (equipment_id, code),
    UNIQUE KEY uk_poc_node (node_id)    -- Each network node can only be one PoC
);

-- Critical Error Detection Views and Queries

-- View: Equipment PoCs without utility (CRITICAL ERROR)
CREATE VIEW v_critical_pocs_no_utility AS
SELECT 
    e.fab,
    e.toolset_code,
    e.phase,
    ...
    p.code as poc_code,
    p.node_id,
    'PoC without utility assignment' as error_reason
FROM tb_equipment e
JOIN tb_equipment_pocs p ON e.id = p.equipment_id
WHERE p.is_used = TRUE 
AND p.utility_code IS NULL
AND p.is_active = TRUE
AND e.is_active = TRUE;

-- View: Missing PoC nodes in network (CRITICAL ERROR)
CREATE VIEW v_critical_missing_poc_nodes AS
SELECT 
    e.fab,
    e.toolset_code, 
    e.phase,
    ...
    p.code as poc_code,
    p.node_id,
    'PoC node missing from nw_nodes' as error_reason
FROM tb_equipment e
JOIN tb_equipment_pocs p ON e.id = p.equipment_id
LEFT JOIN nw_nodes n ON p.node_id = n.id
WHERE n.id IS NULL
AND p.is_active = TRUE
AND e.is_active = TRUE;

-- View: Equipment virtual nodes missing from network (CRITICAL ERROR)
CREATE VIEW v_critical_missing_equipment_nodes AS
SELECT 
    e.fab,
    e.toolset_code,
    e.phase,
    e.name as equipment_name,
    e.node_id,
    'Equipment virtual node missing from nw_nodes' as error_reason
FROM tb_equipment e
LEFT JOIN nw_nodes n ON e.node_id = n.id
WHERE n.id IS NULL
AND e.is_active = TRUE;

-- Random Selection Strategy Functions

-- Function to get random fab (if not specified)
-- SELECT fab FROM tb_toolsets WHERE is_active = TRUE GROUP BY fab ORDER BY RAND() LIMIT 1;

-- Function to get random toolset for fab
-- SELECT code FROM tb_toolsets WHERE fab = ? AND is_active = TRUE ORDER BY RAND() LIMIT 1;

-- Function to get random toolset by code (all phases)
-- SELECT fab, code, phase FROM tb_toolsets WHERE code = ? AND is_active = TRUE ORDER BY RAND() LIMIT 1;

-- Function to get random equipment from toolset
-- SELECT id FROM tb_equipment WHERE fab = ? AND toolset_code = ? AND phase = ? AND is_active = TRUE ORDER BY RAND() LIMIT 1;

-- Function to get random PoCs from equipment
-- SELECT node_id FROM tb_equipment_pocs WHERE equipment_id = ? AND is_active = TRUE ORDER BY RAND() LIMIT 2;

-- Data Population Strategy
CREATE TABLE tb_data_sync_log (
    id INTEGER AUTO_INCREMENT PRIMARY KEY,
    sync_type VARCHAR(32) NOT NULL,    -- FULL_SYNC, INCREMENTAL, VALIDATION
...
CREATE INDEX idx_toolsets_active ON tb_toolsets(is_active, fab, phase);
CREATE INDEX idx_equipment_active ON tb_equipment(is_active, fab, toolset_code, phase);
CREATE INDEX idx_pocs_selection ON tb_equipment_pocs(is_active, is_used, utility_code);

-- Phase Mapping for Compatibility
CREATE TABLE tb_phase_mapping (
    legacy_phase VARCHAR(8),
    standard_phase VARCHAR(8),
    description VARCHAR(128)
);

INSERT INTO tb_phase_mapping VALUES
('A', 'PHASE1', 'Phase A maps to Phase 1'),
('B', 'PHASE2', 'Phase B maps to Phase 2'),
('PHASE1', 'PHASE1', 'Standard Phase 1'),
('PHASE2', 'PHASE2', 'Standard Phase 2');
```
