...
class NetworkPathFinder:
    ...
    def load_network_data(self, start_node_id: int, utility_no: int = 0, toolset_id: int = 0, 
        ...
        try:
            ...
            print(f'  - Total nodes in memory: {len(self.all_nodes)} (much more efficient!)')
            
        except Exception as e:
            print(f'Error loading filtered nodes: {e}')
            raise
        
        # Load links between traversable nodes (much more efficient)
        # We only need links where at least one end is in our traversable nodes
        ...
        links_query = f'''
            ...
            WHERE l.start_node_id IN ({placeholders})
               OR l.end_node_id IN ({placeholders})
        '''
        ...
        try:
            ...
            for row in link_rows:
                ...
                # Collect any node IDs we haven't loaded yet
                if start_node_id not in self.all_nodes:
                    additional_node_ids.add(start_node_id)
                if end_node_id not in self.all_nodes:
                    additional_node_ids.add(end_node_id)
            
            # Load any additional nodes we need for endpoint classification
            if additional_node_ids:
                ...
                additional_rows = self.db.query(additional_nodes_query, list(additional_node_ids))
                ...
                print(f'✓ Loaded {len(additional_node_ids)} additional nodes for endpoint classification')
