...
class CoverageService:
    ...
    def get_coverage_gaps(self, building_code: str, min_gap_size: int = 5) -> List[dict]:
        ...
        # Group uncovered nodes by proximity (simplified approach)
        uncovered_nodes = uncovered_info['uncovered_nodes']
        if len(uncovered_nodes) >= min_gap_size:
            # Simple gap detection - consecutive node IDs
            consecutive_groups = []
            current_group = [uncovered_nodes[0]] if uncovered_nodes else []
            ...
            for i in range(1, len(uncovered_nodes)):
                if uncovered_nodes[i] == uncovered_nodes[i-1] + 1:
                    ...
                    current_group = [uncovered_nodes[i]]
    ...
    def _get_building_totals(self, building_code: str) -> tuple:
        """Get total node and link counts for a building."""
        if not building_code or building_code == "SCENARIO":
            # For scenarios or empty building codes, return dummy values
            return 0, 0
        ...
        try:
            ...
            node_count = node_result[0][0] if node_result else 0
    ...
    def _get_all_nodes(self, building_code: str) -> List[int]:
        """Get all node IDs for a building."""
        if not building_code or building_code == "SCENARIO":
            return []
        ...
        try:
            ...
            print(f"Error getting nodes for building {building_code}: {e}")
            return []
    
    def _get_all_links(self, building_code: str) -> List[int]:
        """Get all link IDs for a building."""
        if not building_code or building_code == "SCENARIO":
            return []
        ...
        try:
            ...
            print(f"Error getting links for building {building_code}: {e}")
            return []
    ...
    def _count_covered_nodes_in_category(self, category: str, building_code: str) -> int:
        ...
        try:
            ...
            placeholders = ','.join(['?'] * len(self._covered_nodes))
            sql = f"""
            SELECT COUNT(*) FROM nw_nodes 
            WHERE building_code = ? AND category = ? AND id IN ({placeholders})
            """
            params = [building_code, category] + list(self._covered_nodes)
    ...
    def _count_covered_links_in_category(self, category: str, building_code: str) -> int:
        ...
        try:
            ...
            placeholders = ','.join(['?'] * len(self._covered_links))
            sql = f"""
            SELECT COUNT(*) FROM nw_links 
            WHERE building_code = ? AND category = ? AND id IN ({placeholders})
            """
            params = [building_code, category] + list(self._covered_links)
