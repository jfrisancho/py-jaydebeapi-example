...
Final version with simplified fab-based implementation.
"""
...
from enums import Approach, Method, RunStatus, ExecutionMode
...
def execute_run_with_config(config: RunConfig, verbose: bool = False) -> RunResult:
    ...
    try:
        ...
        if config.approach == Approach.RANDOM and config.fab:
...
def default_mode(args) -> None:
    ...
    try:
        ...
        if args.verbose:
            ...
            if approach == Approach.RANDOM:
                fab_display = f" for {fab}" if fab else ""
...
def interactive_mode():
    ...
    try:
        ...
        if approach == Approach.RANDOM:
            ...
            available_phases = fetch_available_phases(db, fab)
...
if __name__ == "__main__":
    main() Results stored with run ID: {result.run_id}")
            sys.exit(0)
        else:
            print(f"\n❌ Analysis failed. Check logs for details.")
            sys.exit(1)
